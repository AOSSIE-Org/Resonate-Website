...
const Features = () => {
  return (
    <section className="features mx-auto max-w-[1000px] px-5 py-16">
      {featuresData.map((feature) => (
        <div key={feature.id} className={`feature-item flex flex-col items-center justify-between gap-8 md:flex-row md:gap-16 md:mb-24 mb-16 ${feature.align === 'right' ? 'md:flex-row-reverse' : ''}`}>
          ...
          <div className="feature-content flex-1">
            <h3 className="text-[1.8rem] font-semibold leading-[1.4] text-[#1a1a1a]"><span className="feature-number text-[0.85em]">{feature.id}.</span> {feature.title}</h3>
          </div>
        </div>
      ))}
    </section>
